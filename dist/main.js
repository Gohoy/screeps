...
const spawn0 = function (roomName) {
  ...
  if (upgraders.length < 1 && harvesters.length >= 2 && carriers.length >= 1) {
    ...
    spawn.spawnCreep([WORK, MOVE, CARRY], newName, {
      memory: { role: "upgrader", harvesting: true, help: "0" },
    });
    // 这个是全速升级的配置
  ...
  if (
    ...
    spawn.spawnCreep(
      ...
      newName,
      { memory: { role: "builder", harvesting: true, help: "0" } }
    );
  }
  ...
  if (harvesters.length < 2) {
    ...
    var harvesters0 = harvesters.filter((harvester) => {
      return harvester.memory.source == structures.source0;
    });
    var harvesters1 = harvesters.filter((harvester) => {
      return harvester.memory.source == structures.source1;
    });
      ...
      spawn.spawnCreep(
        ...
        newName,
        {
          memory: {
            role: "harvester",
            harvesting: true,
            source: structures.source1,
            target: structures.sourceLink,
      ...
      spawn.spawnCreep(
        ...
        newName,
        {
          memory: {
            role: "harvester",
            harvesting: true,
            source: structures.source0,
            target: structures.sourceContainer,
            resourceType: RESOURCE_ENERGY,
          },
        }
      );
    ...
    spawn.spawnCreep(
      ...
      newName,
      {
        memory: {
          ...
          target: structures.mineralContainer,
          resourceType: minerals[0].mineralType,
        },
      }
    );
  }
  ...
  if (
    carriers.length < 1 &&
    harvesters.length < 1 &&
    harvesterForContainers.length < 4
  ) {
    ...
    if (harvesterForContainers.length >= 2) {
      ...
      spawn.spawnCreep([WORK, CARRY, MOVE, CARRY, MOVE], newName, {
        memory: { role: "harvesterForContainer", harvesting: true, help: "0" },
      });
    }
  }
    ...
    spawn.spawnCreep(
      ...
      newName,
      {
        memory: {
          role: "carrier",
          harvesting: true,
          frombase: structures.sourceContainer,
          tolist: to,
        },
      }
    );
  ...
  if (minerals.length > 0 && mineCarriers.length < 1 && miners.length > 0) {
    ...
    spawn.spawnCreep([CARRY, CARRY, MOVE, MOVE], newName, {
      memory: {
        role: "mineCarrier",
        harvesting: true,
        frombase: structures.mineralContainer,
      },
    });
  }

  if (
    (repairers.length < 1 && room.storage.store[RESOURCE_ENERGY] >= 100000) ||
    (repairers.length < 2 && room.storage.store[RESOURCE_ENERGY] >= 600000)
  ) {
    ...
    spawn.spawnCreep(
      ...
      newName,
      { memory: { role: "repairer", harvesting: true, help: "0" } }
    );
  }
  ...
  if (Game.time % 1000 == 0) {
    ...
    for (var index in toSellResource) {
      ...
      if (
        terminal.store[resource] >= 10000 &&
        !roomSellResourceTypes.includes(resource)
      ) {
        Game.market.createOrder(
          ORDER_SELL,
          resource,
          Game.market.getHistory(resource)[14].avgPrice,
          10000,
          roomName
        );
      }
